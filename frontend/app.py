...
.pipe-step {
    ...
    letter-spacing: 0.05em; color: #6b6880;
...
.stTextInput > div > div > input {
    ...
    padding: 0.85rem 1.2rem !important; transition: border-color 0.2s !important;
}
...
.stTextInput label {
    color: #9d9aad !important; font-family: 'DM Mono', monospace !important;
    font-size: 0.75rem !important; letter-spacing: 0.1em !important;
    text-transform: uppercase !important; margin-bottom: 0.5rem !important;
}
.stButton > button {
    background: linear-gradient(135deg, #7c3aed, #4f46e5) !important;
    color: white !important; border: none !important; border-radius: 10px !important;
    padding: 0.75rem 2.5rem !important; font-family: 'Syne', sans-serif !important;
...
.output-content table {
    ...
    margin: 1rem 0 !important; font-family: 'DM Mono', monospace !important; font-size: 0.8rem !important;
}
.output-content th {
...
.output-content code {
    background: rgba(167,139,250,0.1) !important; color: #c4b5fd !important;
    padding: 0.2rem 0.5rem !important; border-radius: 4px !important;
...
.download-row { display: flex; gap: 1rem; margin-top: 1.5rem; }
</style>
...
def markdown_to_pdf(markdown_text: str, idea: str) -> bytes:
    """Convert markdown output to a clean PDF using fpdf2."""
    try:
        ...
        class PDF(FPDF):
            ...
            def footer(self):
                ...
                self.set_text_color(80, 75, 100)
                self.cell(0, 10, f'Page {self.page_no()} | Generated by AI Product Manager Agent', align='C')
        ...
        for line in lines:
            ...
            # H4
                ...
                pdf.set_text_color(180, 170, 220)
                text = line[5:].strip()
                text = re.sub(r'[^\x00-\x7F]+', '', text)
            ...
            # Table row
                ...
                if all(re.match(r'^:?-+:?$', c.strip()) for c in cells if c.strip()):
                    continue
                is_header = pdf.get_font_size() == 13  # rough header detection

                col_width = 170 / max(len(cells), 1)
                pdf.set_font('Helvetica', 'B' if '**' in line or is_header else '', 8)
                pdf.set_text_color(167, 139, 250) if '**' in line else pdf.set_text_color(184, 181, 200)
                pdf.set_fill_color(30, 20, 50) if '**' in line else pdf.set_fill_color(18, 15, 30)
        ...
        # Fallback: return plain text as bytes if fpdf2 not installed
...
with st.sidebar:
    ...
    if gemini_key and gemini_key.startswith("AIza"):
        ...
        font-family: DM Mono, monospace; font-size: 0.7rem; color: #6ee7b7'>
            ✓ Key looks valid
        </div>
        """, unsafe_allow_html=True)
    elif gemini_key:
        ...
        border-radius: 8px; padding: 0.6rem 0.9rem; margin-top: 0.5rem;
        font-family: DM Mono, monospace; font-size: 0.7rem; color: #fca5a5'>
...
if not gemini_key or not gemini_key.strip():
    ...
    <div class="lock-screen">
        ...
        <div class="lock-sub">
            This app uses the Gemini API to power the AI agent.<br>
            Enter your free Gemini API key in the sidebar to get started.
        </div>
        <a class="lock-link" href="https://aistudio.google.com/app/apikey" target="_blank">
            Get your free key →
        </a>
    </div>
    """, unsafe_allow_html=True)
...
<div class="pipeline">
    <span class="pipe-step">🔍 search_tool</span>
    <span class="pipe-arrow">→</span>
    <span class="pipe-step">📄 rag_tool</span>
    <span class="pipe-arrow">→</span>
    <span class="pipe-step">🧠 research_tool</span>
    <span class="pipe-arrow">→</span>
    <span class="pipe-step">📋 prd_tool</span>
    <span class="pipe-arrow">→</span>
    <span class="pipe-step">📊 planning_tool</span>
    <span class="pipe-arrow">→</span>
...
with st.expander("📎 Attach a research PDF (optional)"):
    ...
    if uploaded_file is not None:
        try:
            ...
            res = requests.post("http://127.0.0.1:8000/upload_pdf", files=files, params={"gemini_api_key": gemini_key})
            if res.status_code == 200:
                st.success("✅ PDF uploaded and indexed successfully!")
            else:
                st.error(f"PDF upload failed: {res.text}")
        except Exception as e:
        ...
        with st.spinner("🤖 Agent is working... This may take 2-3 minutes"):
            try:
                ...
                res = requests.get(
                    ...
                    params={"idea": idea, "gemini_api_key": gemini_key},
                    timeout=400
                )
                ...
                if res.status_code == 200:
                    try:
                        raw = res.json()
                        data = raw["result"] if "result" in raw else raw
                        ...
                        try:
                            raw_text = res.json()
                            output = str(raw_text.get("result", {}).get("output", str(raw_text))) if isinstance(raw_text, dict) else str(raw_text)
                        except:
                            output = res.text
                        ...
                        dl_col1, dl_col2 = st.columns(2)
                        with dl_col1:
                            st.download_button("⬇️ Download as Markdown", data=output, file_name="product_plan.md", mime="text/markdown", use_container_width=True)
                        with dl_col2:
                            pdf_bytes = markdown_to_pdf(output, idea)
                            st.download_button("📄 Download as PDF", data=pdf_bytes, file_name="product_plan.pdf", mime="application/pdf", use_container_width=True)

                elif res.status_code == 401:
                    st.error("🔑 Invalid API key. Please check your Gemini API key in the sidebar.")
                else:
                    st.error(f"Backend error (status {res.status_code}): {res.text[:300]}")

            except requests.exceptions.Timeout:
                st.error("⏱️ Request timed out. Try a simpler idea or wait and retry.")
